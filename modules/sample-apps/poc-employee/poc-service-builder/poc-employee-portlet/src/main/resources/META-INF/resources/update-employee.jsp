...
<%
	...
	System.out.println(hasAccount +" "+ birthDay +" "+ gender +" " + address +" "+ employeeId +" "+name);
...
<aui:form action="<%= updateEmployeeActionURL %>" name="employeeForm" method="POST">
    ...
    <b>Gender</b><br/>
    <input type="radio" <%if(hasAccount.contains("0")){%>checked="checked"<%}%>  name="<portlet:namespace/>gender" value="male">Male<br>
    <input type="radio"  <%if(hasAccount.contains("1")){%>checked="checked"<%}%>  name="<portlet:namespace/>gender" value="female">Female<br/>
    ...
    <aui:input name="address" >
        ...
        <aui:validator name="string"/>
            ...
            <input type="checkbox" <%if(hasAccount.equals("true")){%>checked<%}%> name="<portlet:namespace/>hasAccount" value="true">HasAccount<br>
